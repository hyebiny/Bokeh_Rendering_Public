...
def _crop_fn(data: Dict, top: int, left: int, height: int, width: int) -> Dict:
    ...
    if "bokeh" in data:
        bokeh = data.pop("bokeh")
        data["bokeh"] = F.crop(bokeh, top=top, left=left, height=height, width=width)
...
def _resize_fn(
    ...
    if "bokeh" in data:
        bokeh = data.pop("bokeh")
        data["bokeh"] = F.resize(img=bokeh, size=[size_h, size_w], interpolation=interpolation)
...
class PhotometricDistort(BaseTransformation):
    ...
    def __call__(self, data: Dict) -> Dict:
        ...
        if "bokeh" in data:
            bokeh = data.pop("bokeh")
            data["bokeh"] = self._apply_transformations(bokeh)
        return data
...
class RandomErasing(BaseTransformation, T.RandomErasing):
    ...
    def __call__(self, data: Dict) -> Dict:
        ...
        if "bokeh" in data:
            data["bokeh"] = super().forward(data.pop("bokeh"))
        return data
...
class ToTensor(BaseTransformation):
    ...
    def __call__(self, data: Dict) -> Dict:
        ...
        if "bokeh" in data:
            ...
            data["bokeh"] = img.to(dtype=self.img_dtype).div(255.0)
