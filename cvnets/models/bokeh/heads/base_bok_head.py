...
class BaseBokHead(nn.Module):
    """
    Base class for bokeh heads
    """

    def __init__(self, opts, enc_conf: dict, features=128):

        # features = feature
        use_bn = False

        enc_ch_l5_exp_out = _check_out_channels(enc_conf, "exp_before_cls")
        enc_ch_l5_out = _check_out_channels(enc_conf, "layer5") # int
        enc_ch_l4_out = _check_out_channels(enc_conf, "layer4") # int
        enc_ch_l3_out = _check_out_channels(enc_conf, "layer3") # int
        enc_ch_l2_out = _check_out_channels(enc_conf, "layer2") # int
        enc_ch_l1_out = _check_out_channels(enc_conf, "layer1") # int
        ...
        # mobilevitv2-1.0.pt features = 128, [128, 256, 384, 512]
        # mobilevitv2-2.0.pt features = 256, [256, 512, 768, 1024]
        self.scratch = _make_scratch([features, features*2, features*3, features*4], features, groups=1, expand=False ) # features = 128
    ...
    def forward(self, enc_out: Dict, *args, **kwargs) -> Tensor or Tuple[Tensor]:
        ...
        '''print('----------- hyebin in bokeh/heads/base_bok_head.py')
        print(layer_1.shape)
        print(layer_2.shape)
        print(layer_3.shape)
        print(layer_4.shape)
