...
from . import BaseBokeh, register_bokeh_models
from .heads import build_bokeh_head
from ..classification import BaseEncoder


@register_bokeh_models(name="encoder_decoder")
class BokEncoderDecoder(BaseBokeh):
    """
    This class defines a encoder-decoder architecture for the task of semantic bokeh. Different bokeh
    ...
    def __init__(self, opts, encoder: BaseEncoder, *args, **kwargs) -> None:
        ...
        # delete layers that are not required in bokeh network
        self.encoder.classifier = None
        use_l5_exp = getattr(opts, "model.bokeh.use_level5_exp", False)
        features = getattr(opts, "model.bokeh.features", 128)
        if not use_l5_exp:
            self.encoder.conv_1x1_exp = None

        self.bok_head = build_bokeh_head(
            opts=opts, enc_conf=self.encoder.model_conf_dict, use_l5_exp=use_l5_exp, features = features
        )
    ...
    def get_trainable_parameters(
        ...
        **kwargs
        ...
        """This function separates the parameters for backbone and bokeh head, so that
        different learning rates can be used for backbone and bokeh head
        """
        ...
        decoder_params, dec_lr_mult = self.bok_head.get_trainable_parameters(
            weight_decay=weight_decay, no_decay_bn_filter_bias=no_decay_bn_filter_bias
        )

        total_params = sum([p.numel() for p in self.parameters()])
        encoder_params_count = sum([p.numel() for p in self.encoder.parameters()])
        decoder_params_count = sum([p.numel() for p in self.bok_head.parameters()])
    ...
    def forward(
        ...
        return self.bok_head(enc_out=enc_end_points, *args, **kwargs)

    def profile_model(self, input: Tensor) -> None:
        """
        This function computes layer-wise FLOPs and parameters for bokeh network
        ...
        out, decoder_params, decoder_macs = self.bok_head.profile_module(enc_end_points)
