    ...
    def train_epoch(self, epoch):
        ...
        for batch_id, batch in enumerate(self.train_loader):
            ...
            with autocast(enabled=self.mixed_precision_training):
                ...
                pred_label = pred_label.type(torch.float32)

                # compute loss
                if epoch < 20:
                    loss_fn = torch.nn.L1Loss()
                    loss = loss_fn(input_img, pred_label)
                elif epoch >=20 and epoch < 40:
                    loss_fn = torch.nn.L1Loss()
                    loss = loss_fn(target_label, pred_label)
                else:
                    sys.exit()
                    # in msssim, i will compare target_label and pred_label
                    loss = self.criteria(
                        input_sample=target_label, prediction=pred_label, target=target_label
                    )

                # in msssim, i will compare target_label and pred_label
                # loss_fn = torch.nn.L1Loss()
                # loss = loss_fn(target_label, pred_label)
                # img1 = target_label.clone().detach()
                # img2 = pred_label.clone().detach()
                '''for i in range(pred_label.shape[0]):
                    im1 = pred_label[i].cpu()  # .detach().numpy()
                    im2 = target_label[i].cpu()  # .detach().numpy()
                    save_image(im1, 'during_training2/msssim1_img1_%d.png' % (i))
                    save_image(im2, 'during_training2/msssim1_img2_%d.png' % (i))'''

                # loss = self.criteria(input_sample=target_label, prediction=pred_label, target=target_label)
                # loss = self.criteria(input_sample=img1, prediction=img2, target=target_label)

                '''for i in range(pred_label.shape[0]):
                    im1 = pred_label[i].cpu()  # .detach().numpy()
                    im2 = target_label[i].cpu()  # .detach().numpy()
                    save_image(im1, 'during_training2/msssim2_img1_%d.png' % (i))
                    save_image(im2, 'during_training2/msssim2_img2_%d.png' % (i))
                sys.exit()'''

                if isinstance(loss, torch.Tensor) and torch.isnan(loss):
                    for i in range(pred_label.shape[0]):
                        img = pred_label[i].cpu()  # .detach().numpy()
                        # img = np.transpose(img, (1,2,0))
                        # print(img.shape)
                        save_image(img, 'during_training/pred_%d_%d.png' % (epoch, i))
